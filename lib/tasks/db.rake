namespace :db do
  ...
  def mysql_cli
    return "mysql -u root -h db -P 3306 -pKuttA_2021BD"
  end
  ...
  task :drop do
    sh %q(docker-compose run --rm db /bin/sh -c "mysql -u root -h db -P 3306 -pKuttA_2021BD -e 'DROP DATABASE peatio_production'")
    sh %q(docker-compose run --rm db /bin/sh -c "mysql -u root -h db -P 3306 -pKuttA_2021BD -e 'DROP DATABASE barong_production'")
    sh %q(docker-compose run --rm db /bin/sh -c "mysql -u root -h db -P 3306 -pKuttA_2021BD -e 'DROP DATABASE superset'")
  end
