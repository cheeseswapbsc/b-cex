namespace :service do
  ...
  task :cryptonodes, [:command] do |task, args|
    ...
    def start
      ...
      sh 'docker-compose up -d parity bitcoind litecoind'
    end

    def stop
      puts '----- Stopping cryptonodes -----'
      sh 'docker-compose rm -fs parity bitcoind litecoind'
    end

    @switch.call(args, method(:start), method(:stop))
  end

  desc 'Run setup hooks for peatio and barong for nDAX'
  ...
  desc 'Run nPro app (barong, peatio)'
  ...
  desc 'Run the nPro application'
  task :frontend, [:command] do |task, args|
    args.with_defaults(:command => 'start')

    def start
      puts '----- Starting the nPro -----'
      sh 'docker-compose up -d frontend'
    end

    def stop
      puts '----- Stopping the nPro -----'
  ...
  desc 'Run the nDAX stack with dependencies (does not run Optional)'
