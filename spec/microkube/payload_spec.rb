require 'ndax/payload'

describe Ndax::Payload do
  let(:secret) { 'myTailorIsRich42' }
  let(:encoder) { Ndax::Payload.new(secret: secret) }
  let(:valid_decoder) { encoder }
  let(:invalid_decoder) { Ndax::Payload.new(secret: 'randomSecret') }
  let(:service) { 'barong' }
  let(:image) { 'ndexnetwork/barong:2.0.8-alpha' }
  ...
  it 'defines an API' do
    pgen = Ndax::Payload.new(secret: '0x42', expire: 600)
  ...
  it 'expire time should be configurable' do
    exp_time = 4200
    exp_coder = Ndax::Payload.new(secret: 'ab', expire: exp_time)
